...
export class OrganizationsRepo {
  ...
  async getById(id: string): Promise<Organization | null> {
    const snap = await getFirebaseApp().firestore().collection(COLLECTION).doc(id).get();
    return snap.exists ? (snap.data() as Organization) : null;
  }
  async list(): Promise<Organization[]> {
    const qs = await getFirebaseApp().firestore().collection(COLLECTION).get();
    return qs.docs.map((d) => d.data() as Organization);
  }
  async update(id: string, patch: Partial<Organization>): Promise<Organization | null> {
    const ref = getFirebaseApp().firestore().collection(COLLECTION).doc(id);
  ...
  async remove(id: string): Promise<boolean> {
    await getFirebaseApp().firestore().collection(COLLECTION).doc(id).delete();
    return true;
  }
}
